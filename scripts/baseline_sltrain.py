...
# from peft_pretraining.train_utils import build_model, build_optimizer
...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size):
    _time = time.time()
    val_data = dataloaders.load_dataset("allenai/c4", "en", split="validation", streaming=True) #DGX
    val_data = val_data.shuffle(seed=42)
    logger.info(f"Loaded validation dataset in {time.time() - _time:.2f} seconds")

    if not args.single_gpu:
        val_data = dataloaders.distributed.split_dataset_by_node(val_data, rank=global_rank, world_size=world_size)
...
def main(args):
    ...
    data: dataloaders.Dataset = data.shuffle(seed=seed_for_shuffle)
    if not args.single_gpu:
        data = dataloaders.distributed.split_dataset_by_node(
    ...
    def unwrap(model):
        return getattr(model, "module", getattr(model, "wrapped_model", model))

    def list_self_attn_names(model, include_projections=False):
        # 兼容：直接按模块类型匹配 LlamaAttention；若不可用，则按名称包含 ".self_attn" 匹配
        try:
            from transformers.models.llama.modeling_llama import LlamaAttention
            def is_self_attn(name, mod):
                return isinstance(mod, LlamaAttention)
        except Exception:
            def is_self_attn(name, mod):
                return name.endswith("self_attn") or ".self_attn" in name

        m = unwrap(model)
        names = []

        for name, mod in m.named_modules():
            if is_self_attn(name, mod):
                if not include_projections:
                    # 只要 self_attn 模块本身
                    # 过滤掉它的子模块名（q_proj/k_proj/...）
                    if name.endswith("self_attn"):  # 典型形如 model.layers.0.self_attn
                        names.append(name)
                else:
                    # 同时列出 q/k/v/o
                    names.append(name)
                    for subn, subm in mod.named_modules():
                        if isinstance(subm, nn.Linear):
                            full = f"{name}.{subn}" if subn else name
                            names.append(full)

        return names
    
    for n in list_self_attn_names(model):
        print(n)
