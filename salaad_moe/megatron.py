...
def megatron_arguments(config, data_directory, tokenizer_directory, output, resume=None):
    ...
    values = {
        ...
        "lr-decay-iters": t["total_optimizer_steps"],
        "adam-beta1": t["beta1"],
        "adam-beta2": t["beta2"],
        "adam-eps": t["epsilon"],
        ...
        # Zero iterations disables validation and test. Upstream sample-count
        # arithmetic still requires a positive evaluation interval.
        "eval-interval": t["total_optimizer_steps"] + 1,
        "eval-iters": 0,
        "log-interval": 1,
        "ckpt-format": "torch",
        "num-workers": 2,
...
def install_training_integration(
    training_module, config, data_identity, branch_from_vanilla=False
):
